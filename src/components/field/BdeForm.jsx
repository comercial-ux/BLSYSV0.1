...
const ImageInput = ({ label, onFileChange, fileName, isRequired, isMobile = false }) => {
    ...
    return (
        <>
            <div>
                ...
                <button onClick={handleButtonClick} className="relative mt-1 flex justify-center w-full h-32 px-6 pt-5 pb-6 border-2 border-white/20 border-dashed rounded-md cursor-pointer hover:bg-white/5 transition-colors text-left">
                    <div className="space-y-1 text-center">
                        ...
                        <div className="flex text-sm text-gray-400">
                            <p className="pl-1 truncate max-w-[150px]">{fileName ? fileName : (isMobile ? "Adicionar foto" : "Selecionar foto")}</p>
                        </div>
                    </div>
...
const BdeForm = ({ report, onSave, onClose, jobs, contacts, equipments, isMobile = false }) => {
    ...
    const openJobs = useMemo(() => {
      ...
      return jobs.filter(job => {
        ...
        const hasClient = hasProposal && job.proposal.contacts && job.proposal.contacts.name;
        
        return hasValidStatus && hasProposal && hasClient;
      });
    }, [jobs]);

    const operators = useMemo(() => (contacts || []).filter(c => c.type === 'Colaborador' && c.status === 'Ativo'), [contacts]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const relatedObjects = ['job', 'equipment', 'operator', 'proposal', 'proposals', 'contacts', 'sinaleiro'];
